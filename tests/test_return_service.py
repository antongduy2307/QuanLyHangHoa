...
class ReturnServiceTestCase(unittest.TestCase):
    ...
    def test_quick_return_decimal_quantity_keeps_fractional_stock_and_total(self) -> None:
        return_invoice = self.return_service.create_quick_return_invoice(
            customer_id=None,
            customer_snapshot_name="Khach le",
            return_datetime=datetime(2026, 4, 9, 10, 35, 0),
            items=[{"product_id": self.bao_product_id, "unit_type": UnitType.BAO, "quantity": Decimal("2.5")}],
            handling_mode="REFUND_NOW",
        )

        self.assertEqual(return_invoice.items[0].quantity, Decimal("2.5"))
        self.assertEqual(return_invoice.total_amount, Decimal("250.0"))
        self.assertEqual(self.inventory_service.get_available_quantity(self.bao_product_id, UnitType.BAO), Decimal("2.5"))

    def test_quick_return_decimal_bich_quantity_keeps_fractional_stock_and_total(self) -> None:
        return_invoice = self.return_service.create_quick_return_invoice(
            customer_id=None,
            customer_snapshot_name="Khach le",
            return_datetime=datetime(2026, 4, 9, 10, 36, 0),
            items=[{"product_id": self.bich_product_id, "unit_type": UnitType.BICH, "quantity": Decimal("2.5")}],
            handling_mode="REFUND_NOW",
        )

        self.assertEqual(return_invoice.items[0].quantity, Decimal("2.5"))
        self.assertEqual(return_invoice.total_amount, Decimal("50.0"))
        self.assertEqual(self.inventory_service.get_available_quantity(self.bich_product_id, UnitType.BICH), Decimal("2.5"))
    ...
    def test_update_quick_return_invoice_keeps_id_and_code_and_reapplies_effects(self) -> None:
        self.sales_service.create_invoice(
            customer_id=self.customer_id,
            customer_snapshot_name="Khach return",
            invoice_datetime=datetime(2026, 4, 9, 15, 0, 0),
        ...
        quick_return = self.return_service.create_quick_return_invoice(
            customer_id=self.customer_id,
            customer_snapshot_name="Khach return",
            return_datetime=datetime(2026, 4, 9, 15, 5, 0),
            items=[{"product_id": self.bao_product_id, "unit_type": UnitType.BAO, "quantity": Decimal("2")}],
            handling_mode=ReturnHandlingMode.STORE_CREDIT,
        )
        ...
        updated = self.return_service.update_return_invoice(
            quick_return.id,
            items=[{"product_id": self.bao_product_id, "unit_type": UnitType.BAO, "quantity": Decimal("3"), "unit_price": Decimal("100"), "line_total": Decimal("300")}],
            handling_mode=ReturnHandlingMode.REFUND_NOW,
        ...
        customer = self.customer_service.get_customer(self.customer_id)
        self.assertEqual(updated.id, quick_return.id)
        self.assertEqual(updated.return_code, quick_return.return_code)
        self.assertEqual(updated.return_datetime, datetime(2026, 4, 9, 16, 0, 0))
        self.assertEqual(updated.note, "Sua nhanh")
    ...
    def test_update_return_datetime_syncs_return_ledger_timestamps(self) -> None:
        invoice = self.sales_service.create_invoice(
            customer_id=self.customer_id,
            customer_snapshot_name="Khach return",
            invoice_datetime=datetime(2026, 4, 9, 17, 0, 0),
            items=[{"product_id": self.bao_product_id, "unit_type": UnitType.BAO, "quantity": Decimal("4")}],
            paid_amount=Decimal("100"),
        )
        source_item = invoice.items[0]
        return_invoice = self.return_service.create_return_invoice(
            source_invoice_id=invoice.id,
            return_datetime=datetime(2026, 4, 9, 17, 15, 0),
            items=[{"source_invoice_item_id": source_item.id, "quantity": Decimal("2")}],
            handling_mode=ReturnHandlingMode.REFUND_NOW,
        )
        original_ledgers = [(ledger.event_type, ledger.amount_delta, ledger.balance_after) for ledger in self._ledger_for_return(return_invoice.id)]
        ...
        updated = self.return_service.update_return_datetime(return_invoice.id, datetime(2026, 4, 10, 8, 0, 0))

        reloaded = self.returns_repository.get_return_invoice(return_invoice.id)
        updated_ledgers = [(ledger.event_type, ledger.amount_delta, ledger.balance_after) for ledger in self._ledger_for_return(return_invoice.id)]
        customer = self.customer_service.get_customer(self.customer_id)
        self.assertEqual(updated.return_datetime, datetime(2026, 4, 10, 8, 0, 0))
        self.assertEqual(reloaded.return_datetime, datetime(2026, 4, 10, 8, 0, 0))
        self.assertEqual(reloaded.total_amount, return_invoice.total_amount)
        ...
        self.assertTrue(all(ledger.transaction_datetime == datetime(2026, 4, 10, 8, 0, 0) for ledger in self._ledger_for_return(return_invoice.id)))
        self.assertEqual(self.inventory_service.get_available_quantity(self.bao_product_id, UnitType.BAO), original_stock)

    def test_delete_return_invoice_rolls_back_inventory_and_customer_effects(self) -> None:
        invoice = self.sales_service.create_invoice(
            customer_id=self.customer_id,
            customer_snapshot_name="Khach return",
            invoice_datetime=datetime(2026, 4, 9, 17, 0, 0),
        ...
        self.return_service.delete_return_invoice(return_invoice.id)

        with self.assertRaises(NotFoundError):
            self.returns_repository.get_return_invoice(return_invoice.id)
        customer = self.customer_service.get_customer(self.customer_id)
        self.assertEqual(customer.current_balance, Decimal("300"))
        self.assertEqual(customer.total_sales, Decimal("400"))
        self.assertEqual(self.inventory_service.get_available_quantity(self.bao_product_id, UnitType.BAO), Decimal("-4"))
        self.assertEqual(self._ledger_for_return(return_invoice.id), [])
