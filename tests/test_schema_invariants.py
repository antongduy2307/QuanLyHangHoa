...
from modules.inventory.models import InventoryAdjustment, InventoryAdjustmentItem, InventoryBalance, Product
...
class SchemaInvariantTestCase(unittest.TestCase):
    ...
    def test_return_invoice_item_allows_optional_source_link_for_quick_returns(self) -> None:
        with self.Session() as session:
            ...
            quick_return_item_shape = ReturnInvoiceItem(
            ...
            session.add(quick_return_item_shape)
            session.commit()
            self.assertIsNone(quick_return_item_shape.source_invoice_item_id)
    ...
    def test_inventory_adjustment_item_allows_negative_old_quantity_but_keeps_new_quantity_non_negative(self) -> None:
        with self.Session() as session:
            product = self._make_product("P-ADJ", UnitMode.BAO_KG)
            adjustment = InventoryAdjustment()
            session.add_all([product, adjustment])
            session.flush()

            item = InventoryAdjustmentItem(
                adjustment_id=adjustment.id,
                product_id=product.id,
                old_quantity=Decimal("-14"),
    ...
    def test_inventory_adjustment_item_still_rejects_negative_new_quantity(self) -> None:
        with self.Session() as session:
            product = self._make_product("P-ADJ-2", UnitMode.BAO_KG)
            adjustment = InventoryAdjustment()
            session.add_all([product, adjustment])
            session.flush()
