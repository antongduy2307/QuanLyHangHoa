...
class InventoryServiceTestCase(unittest.TestCase):
    ...
    def test_create_product_bich(self) -> None:
        ...
        self.repository.session.commit()
        balance = self.service.get_balance(product.id)
        self.assertEqual(balance.on_hand_bich_integer, Decimal("0"))
    ...
    def test_bich_increase_and_decrease_keep_decimal_stock(self) -> None:
        self.service.increase_stock(self.bich_product_id, Decimal("3.5"), UnitType.BICH)
        self.assertEqual(self.service.get_available_quantity(self.bich_product_id, UnitType.BICH), Decimal("3.5"))

        self.service.decrease_stock(self.bich_product_id, Decimal("5.25"), UnitType.BICH)
        self.assertEqual(self.service.get_available_quantity(self.bich_product_id, UnitType.BICH), Decimal("-1.75"))
